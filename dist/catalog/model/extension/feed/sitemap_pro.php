...
class ModelExtensionFeedSitemapPro extends Model {
	...
	public function getProducts() {
		...
		if (!$product_data) {
			$sql = "SELECT p.product_id, p.image, pd.name  FROM product p LEFT JOIN product_description pd ON (p.product_id = pd.product_id) LEFT JOIN product_to_store p2s ON (p.product_id = p2s.product_id)"; 
