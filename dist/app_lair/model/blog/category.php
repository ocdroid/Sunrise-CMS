...
class ModelBlogCategory extends Model
{
    public function addCategory($data)
    {
        ...
        foreach ($data['category_description'] as $language_id => $value) {
            $this->db->query("
                ...
                SET blog_category_id = '" . (int)$blog_category_id . "', 
        ...
        $query = $this->db->query("
            SELECT * 
            FROM `blog_category_path` WHERE blog_category_id = '" . (int)$data['parent_id'] . "' 
            ORDER BY `level` ASC
        ");

        foreach ($query->rows as $result) {
            $this->db->query("
                INSERT INTO `blog_category_path` 
                SET `blog_category_id` = '" . (int)$blog_category_id . "', 
        ...
        if (isset($data['category_store'])) {
            foreach ($data['category_store'] as $store_id) {
                $this->db->query("
                    INSERT INTO blog_category_to_store 
                    SET blog_category_id = '" . (int)$blog_category_id . "', 
        ...
        if (isset($data['category_layout'])) {
            foreach ($data['category_layout'] as $store_id => $layout_id) {
                $this->db->query("
                    INSERT INTO blog_category_to_layout 
                    SET blog_category_id = '" . (int)$blog_category_id . "', 
                        store_id = '" . (int)$store_id . "', 
        ...
        if (isset($data['keyword']) && !empty($data['keyword'])) {
            $this->db->query("
                ...
                SET query = 'blog_category_id=" . (int)$blog_category_id . "', 
    ...
    public function editCategory($blog_category_id, $data)
    {
        ...
        $query = $this->db->query("
            ...
            WHERE path_id = '" . (int)$blog_category_id . "' 
            ORDER BY level ASC
        ");
        ...
        if ($query->rows) {
			...
			FROM `blog_category_path` 
                ...
                $query = $this->db->query("
                    SELECT * 
                    FROM `blog_category_path` 
                    WHERE blog_category_id = '" . (int)$data['parent_id'] . "' 
                    ORDER BY level ASC
                ");
                ...
                foreach ($path as $path_id) {
                    $this->db->query("
                        REPLACE INTO `blog_category_path` 
                        SET blog_category_id = '" . (int)$category_path['blog_category_id'] . "', 
                            `path_id` = '" . (int)$path_id . "', 
                            level = '" . (int)$level . "'
                    ");
    ...
    public function repairCategories($parent_id = 0)
    {
        ...
        foreach ($query->rows as $category) {
            ...
            $query = $this->db->query("
                SELECT * 
                FROM `blog_category_path` 
                WHERE blog_category_id = '" . (int)$parent_id . "' 
                ORDER BY level ASC
            ");
            ...
            foreach ($query->rows as $result) {
                $this->db->query("
                    INSERT INTO `blog_category_path` 
                    SET blog_category_id = '" . (int)$category['blog_category_id'] . "', 
                        `path_id` = '" . (int)$result['path_id'] . "', 
                        level = '" . (int)$level . "'
                ");
            ...
            $this->db->query("
                REPLACE INTO `blog_category_path` 
                SET blog_category_id = '" . (int)$category['blog_category_id'] . "', 
                    `path_id` = '" . (int)$category['blog_category_id'] . "', 
                    level = '" . (int)$level . "'
            ");
        ...
        $query = $this->db->query("
            SELECT DISTINCT *, 
                (
                    ...
                    FROM blog_category_path cp 
                    LEFT JOIN blog_category_description cd1 ON (cp.path_id = cd1.blog_category_id AND cp.blog_category_id != cp.path_id) 
                    WHERE cp.blog_category_id = c.blog_category_id 
                        AND cd1.language_id = '" . (int)$this->config->get('config_language_id') . "' 
                    GROUP BY cp.blog_category_id) AS path, 
                (
                    SELECT DISTINCT keyword 
                    FROM url_alias WHERE query = 'blog_category_id=" . (int)$blog_category_id . "') AS keyword 
            FROM blog_category c 
            LEFT JOIN blog_category_description cd2 ON (c.blog_category_id = cd2.blog_category_id) 
    ...
    public function getCategories($data = array())
    {
        $sql = "
            SELECT cp.blog_category_id AS blog_category_id, 
                ...
                c1.noindex 
            FROM blog_category_path cp 
            LEFT JOIN blog_category c1 ON (cp.blog_category_id = c1.blog_category_id) 
            LEFT JOIN blog_category c2 ON (cp.path_id = c2.blog_category_id) 
            LEFT JOIN blog_category_description cd1 ON (cp.path_id = cd1.blog_category_id) 
            LEFT JOIN blog_category_description cd2 ON (cp.blog_category_id = cd2.blog_category_id) 
    ...
    public function getCategoryStores($blog_category_id)
    {
        ...
        $query = $this->db->query("
            SELECT * FROM blog_category_to_store 
            WHERE blog_category_id = '" . (int)$blog_category_id . "'
        ");
        ...
        $query = $this->db->query("
            SELECT *, 
                (
                    ...
                    FROM blog_category 
                    WHERE parent_id = c.blog_category_id) AS children 
            FROM blog_category c 
