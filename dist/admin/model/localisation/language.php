...
class ModelLocalisationLanguage extends Model {
	public function addLanguage($data) {
		...
		foreach ($query->rows as $blog_category) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "blog_category_description SET blog_category_id = '" . (int)$blog_category['blog_category_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($blog_category['name']) . "', description = '" . $this->db->escape($blog_category['description']) . "', meta_description = '" . $this->db->escape($blog_category['meta_description']) . "', meta_title = '" . $this->db->escape($blog_category['meta_title']) . "', meta_h1 = '" . $this->db->escape($blog_category['meta_h1']) . "'");
		}
