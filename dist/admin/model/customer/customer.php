...
class ModelCustomerCustomer extends Model
{
    ...
    public function editCustomer($customer_id, $data)
    {
        ...
        if ($data['password']) {
            $this->db->query("UPDATE customer SET salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "' WHERE customer_id = '" . (int)$customer_id . "'");
        }
    ...
    {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM customer_reward WHERE order_id = '" . (int)$order_id . "' AND points > 0");
    ...
    public function getIps($customer_id, $start = 0, $limit = 10)
    {
        ...
        $query = $this->db->query("SELECT * FROM customer_ip WHERE customer_id = '" . (int)$customer_id . "' ORDER BY date_added DESC LIMIT " . (int)$start . "," . (int)$limit);
    ...
    {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM customer_ip WHERE customer_id = '" . (int)$customer_id . "'");
    ...
    {
        $query = $this->db->query("SELECT * FROM `customer_login` WHERE `email` = '" . $this->db->escape($email) . "'");
    ...
    {
        $this->db->query("DELETE FROM `customer_login` WHERE `email` = '" . $this->db->escape($email) . "'");
    }
}
