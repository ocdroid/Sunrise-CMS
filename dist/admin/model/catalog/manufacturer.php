...
class ModelCatalogManufacturer extends Model
{
    ...
    {
        $this->db->query("INSERT INTO manufacturer SET name = '" . $this->db->escape($data['name']) . "', sort_order = '" . (int)$data['sort_order'] . "', noindex = '" . (int)$data['noindex'] . "'");
        
        $manufacturer_id = $this->db->getLastId();
        
        if (isset($data['manufacturer_layout'])) {
            foreach ($data['manufacturer_layout'] as $store_id => $layout_id) {
                $this->db->query("INSERT INTO manufacturer_to_layout SET manufacturer_id = '" . (int)$manufacturer_id . "', store_id = '" . (int)$store_id . "', layout_id = '" . (int)$layout_id . "'");
            }
        }
        ...
        foreach ($data['manufacturer_description'] as $language_id => $value) {
            $this->db->query("INSERT INTO manufacturer_description SET manufacturer_id = '" . (int)$manufacturer_id . "', language_id = '" . (int)$language_id . "', description = '" . $this->db->escape($value['description']) . "', description_bottom = '" . $this->db->escape($value['description_bottom']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "'");
        }
        ...
        if (isset($data['product_related'])) {
            foreach ($data['product_related'] as $related_id) {
                $this->db->query("INSERT INTO product_related_mn SET manufacturer_id = '" . (int)$manufacturer_id . "', product_id = '" . (int)$related_id . "'");
            }
        }
    
        if (isset($data['article_related'])) {
            foreach ($data['article_related'] as $related_id) {
                $this->db->query("INSERT INTO article_related_mn SET manufacturer_id = '" . (int)$manufacturer_id . "', article_id = '" . (int)$related_id . "'");
            }
        }
    ...
    public function editManufacturer($manufacturer_id, $data)
    {
        ...
        $this->db->query("UPDATE manufacturer SET name = '" . $this->db->escape($data['name']) . "', sort_order = '" . (int)$data['sort_order'] . "', noindex = '" . (int)$data['noindex'] . "' WHERE manufacturer_id = '" . (int)$manufacturer_id . "'");
        ...
        $this->db->query("DELETE FROM product_related_mn WHERE manufacturer_id = '" . (int)$manufacturer_id . "'");
    
        if (isset($data['product_related'])) {
            foreach ($data['product_related'] as $related_id) {
                $this->db->query("DELETE FROM product_related_mn WHERE manufacturer_id = '" . (int)$manufacturer_id . "' AND product_id = '" . (int)$related_id . "'");
                $this->db->query("INSERT INTO product_related_mn SET manufacturer_id = '" . (int)$manufacturer_id . "', product_id = '" . (int)$related_id . "'");
            }
        }
        
        $this->db->query("DELETE FROM article_related_mn WHERE manufacturer_id = '" . (int)$manufacturer_id . "'");
    
        if (isset($data['article_related'])) {
            foreach ($data['article_related'] as $related_id) {
                $this->db->query("DELETE FROM article_related_mn WHERE manufacturer_id = '" . (int)$manufacturer_id . "' AND article_id = '" . (int)$related_id . "'");
                $this->db->query("INSERT INTO article_related_mn SET manufacturer_id = '" . (int)$manufacturer_id . "', article_id = '" . (int)$related_id . "'");
            }
        }
    ...
    {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM manufacturer_to_layout WHERE layout_id = '" . (int)$layout_id . "'");
    ...
    public function getProductRelated($manufacturer_id)
    {
        ...
        $query = $this->db->query("SELECT * FROM product_related_mn WHERE manufacturer_id = '" . (int)$manufacturer_id . "'");
    ...
    public function getArticleRelated($manufacturer_id)
    {
        ...
        $query = $this->db->query("SELECT * FROM article_related_mn WHERE manufacturer_id = '" . (int)$manufacturer_id . "'");
