...
class ModelDesignBanner extends Model {
		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $language_id => $value) {
				foreach ($value as $banner_image) {
					$this->db->query("INSERT INTO banner_image SET banner_id = '" . (int)$banner_id . "', language_id = '" . (int)$language_id . "', title = '" .  $this->db->escape($banner_image['title']) . "', link = '" .  $this->db->escape($banner_image['link']) . "', image = '" .  $this->db->escape($banner_image['image']) . "', sort_order = '" .  (int)$banner_image['sort_order'] . "'");
				}
			}
		}
	...
	public function getBannerImages($banner_id) {
		...
		$banner_image_query = $this->db->query("SELECT * FROM banner_image WHERE banner_id = '" . (int)$banner_id . "' ORDER BY sort_order ASC");
